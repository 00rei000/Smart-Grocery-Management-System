...
import { inventoryApi } from '../services/user/api';
...
export function useInventory() {
  ...
  const addMutation = useMutation({
    ...
    onError: () => {
      setError('Unable to add new food item');
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: () => {
      setError('Unable to update food item');
    },
  });
  ...
  const addItem = async (item: Omit<InventoryItem, 'id'>) => {
    try {
      const response = await addMutation.mutateAsync(item);
      return response.data;
    } catch (err) {
